...
branches = st.dictionaries(zaids, st.floats(min_value=1e-10, max_value=1.0)).map(
    lambda d: {k: v / sum(d.values()) for k, v in d.items()}
)
proto = st.builds(ProtoReaction, zaids, branching=st.none() | branches, spectra=st.just(tuple()))
reac = st.builds(Reaction, proto, zaids)
preac = st.builds(ProductionReaction, zaids, zaids, st.text())
particles = st.builds(Particle, st.integers(min_value=0, max_value=4), st.integers(min_value=0, max_value=4), st.text())
rcategories = st.builds(ReactionCategory, st.text(), st.dictionaries(particles, st.integers(min_value=1, max_value=3)))
rates = st.builds(
    ReactionRate,
    st.text(),
